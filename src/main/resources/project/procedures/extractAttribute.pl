...
$opts->{attribute}        = ($ec->getProperty("attribute"))->findvalue('//value')->string_value;
$opts->{attribute_outpp}  = ($ec->getProperty("attribute_outpp"))->findvalue('//value')->string_value;
$opts->{attribute_outpsp} = ($ec->getProperty("attribute_outpsp"))->findvalue('//value')->string_value;
$opts->{xml_outpp}        = ($ec->getProperty("xml_outpp"))->findvalue('//value')->string_value;

$[/myProject/procedure_helpers/preamble]

$xmledit->extract_attribute();
exit($opts->{exitcode});
