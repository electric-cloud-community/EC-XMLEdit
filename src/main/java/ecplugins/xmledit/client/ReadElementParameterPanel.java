...
// ReadElementParameterPanel.java --
//
// ReadElementParameterPanel.java is part of ElectricCommander.
//
...
public class ReadElementParameterPanel
        ...
        ParameterPanelProvider
{
    ...
    // ~ Static fields/initializers----------------------------
    private static UiBinder<Widget, ReadElementParameterPanel> s_binder = GWT
            .create(Binder.class);
    ...
    // ~ Instance fields
    // --------------------------------------------------------
    @UiField FormBuilder readElementParameterForm;
    ...
    @Override public Widget doInit()
    {
        ...
        readElementParameterForm.addRow(false, "Source:",
            "Select the source for the xml to parse.", SOURCE, FILEPATH,
            source);
        readElementParameterForm.addRow(false, "Filepath:",
            "Path to the XML file to parse. ", FILEPATH, "", new TextBox());
        readElementParameterForm.addRow(false, "Property path:",
            "Path to the property where the xml is located. ", PROPERTY, "",
            new TextBox());
        readElementParameterForm.addRow(false, "Code:", "XML code to parse.",
            XML_CODE, "", new TextArea());
        readElementParameterForm.addRow(true, "Xpath Query:",
            "XPath query to get the element.", QUERY, "", new TextArea());
        readElementParameterForm.addRow(true, "Return Type:",
            "Return the entire element tag or just its value.", TYPE, "value",
            return_type);
        readElementParameterForm.addRow(false, "Element Selection:",
            "Select the action to do if the query returns several elements.",
            SELECTION, "first", selection);
        readElementParameterForm.addRow(false,
            "Result Elements(output property sheet path):",
            "Path to property sheet to hold the XML elements read.",
            ELEMENT_OUTPSP, "", new TextBox());
        readElementParameterForm.addRow(false,
            "Result Element(output property path):",
            "Path to property to hold the XML elements read.", ELEMENT_OUTPP,
            "", new TextBox());
    ...
    {
        boolean validationStatus = readElementParameterForm.validate();
        String  source           = readElementParameterForm.getValue(SOURCE);

        // QUERY is required.
        if (StringUtil.isEmpty(
                    readElementParameterForm.getValue(QUERY)
                                        .trim())) {
            readElementParameterForm.setErrorMessage(QUERY,
        ...
        if (FILEPATH.equals(source)) {
            ...
            if (StringUtil.isEmpty(
                        readElementParameterForm.getValue(FILEPATH)
                                            .trim())) {
                readElementParameterForm.setErrorMessage(FILEPATH,
                    "This Field is required.");
                validationStatus = false;
            }
        }
            ...
            if (StringUtil.isEmpty(
                        readElementParameterForm.getValue(PROPERTY)
                                            .trim())) {
                readElementParameterForm.setErrorMessage(PROPERTY,
                    "This Field is required.");
                validationStatus = false;
            }
        }
            ...
            if (StringUtil.isEmpty(
                        readElementParameterForm.getValue(XML_CODE)
                                            .trim())) {
                readElementParameterForm.setErrorMessage(XML_CODE,
                    "This Field is required.");
                validationStatus = false;
            }
        }
    ...
    {
        String source    = readElementParameterForm.getValue(SOURCE);
        String selection = readElementParameterForm.getValue(SELECTION);

        readElementParameterForm.setRowVisible(FILEPATH,
            FILEPATH.equals(source));
        readElementParameterForm.setRowVisible(PROPERTY,
            PROPERTY.equals(source));
        readElementParameterForm.setRowVisible(XML_CODE,
            XML_CODE.equals(source));
        readElementParameterForm.setRowVisible(ELEMENT_OUTPSP,
            "all".equals(selection));
        readElementParameterForm.setRowVisible(ELEMENT_OUTPP,
    ...
    @Override public Map<String, String> getValues()
    {
        ...
        Map<String, String> readFormValues =
            readElementParameterForm.getValues();
        ...
        actualParams.put(ELEMENT_OUTPP, readFormValues.get(ELEMENT_OUTPP));
        actualParams.put(ELEMENT_OUTPSP, readFormValues.get(ELEMENT_OUTPSP));
            ...
            Collection<ActualParameter> actualParameters)
    {
        ...
        for (String key : new String[] {
            ...
            readElementParameterForm.setValue(key,
    ...
    // -------------------------------------------------------
    interface Binder
        extends UiBinder<Widget, ReadElementParameterPanel> { }
}
