...
// UpdateAttributeParameterPanel.java --
//
// UpdateAttributeParameterPanel.java is part of ElectricCommander.
//
...
public class UpdateAttributeParameterPanel
        ...
        ParameterPanelProvider
{
    ...
    // ~ Static fields/initializers----------------------------
    private static UiBinder<Widget, UpdateAttributeParameterPanel> s_binder =
    ...
    // ~ Instance fields
    // --------------------------------------------------------
    @UiField FormBuilder updateAttributeParameterForm;
    ...
    @Override public Widget doInit()
    {
        ...
        updateAttributeParameterForm.addRow(false, "Source:",
            "Select the source for the xml to parse.", SOURCE, FILEPATH,
            source);
        updateAttributeParameterForm.addRow(false, "Filepath:",
            "Path to the XML file to parse. ", FILEPATH, "", new TextBox());
        updateAttributeParameterForm.addRow(false, "Property path:",
            "Path to the property where the xml is located. ", PROPERTY, "",
            new TextBox());
        updateAttributeParameterForm.addRow(false, "Code:",
            "XML code to parse.", XML_CODE, "", new TextArea());
        updateAttributeParameterForm.addRow(true, "Xpath Query:",
            "XPath query to get the element.", QUERY, "", new TextArea());
        updateAttributeParameterForm.addRow(true, "Attribute Name:",
            "Name for the attribute to update.", ATTRIBUTE_NAME, "",
            new TextBox());
        updateAttributeParameterForm.addRow(false, "Attribute Value:",
            "New value for the attribute.", ATTRIBUTE_VALUE, "",
            new TextArea());
        updateAttributeParameterForm.addRow(false, "Update:",
            "Select the action to do if the query returns several elements.",
            SELECTION, "first", selection);
        updateAttributeParameterForm.addRow(false,
    ...
    {
        boolean validationStatus = updateAttributeParameterForm.validate();
        String  source           = updateAttributeParameterForm.getValue(
                SOURCE);

        // QUERY is required.
        if (StringUtil.isEmpty(
                    updateAttributeParameterForm.getValue(QUERY)
                                            .trim())) {
            updateAttributeParameterForm.setErrorMessage(QUERY,
        ...
        if (StringUtil.isEmpty(
                    updateAttributeParameterForm.getValue(ATTRIBUTE_NAME)
                                            .trim())) {
            updateAttributeParameterForm.setErrorMessage(ATTRIBUTE_NAME,
                "This Field is required.");
            validationStatus = false;
        }
        ...
        if (FILEPATH.equals(source)) {
            ...
            if (StringUtil.isEmpty(
                        updateAttributeParameterForm.getValue(FILEPATH)
                                                .trim())) {
                updateAttributeParameterForm.setErrorMessage(FILEPATH,
                    "This Field is required.");
                validationStatus = false;
            }
        }
            ...
            if (StringUtil.isEmpty(
                        updateAttributeParameterForm.getValue(PROPERTY)
                                                .trim())) {
                updateAttributeParameterForm.setErrorMessage(PROPERTY,
                    "This Field is required.");
                validationStatus = false;
            }
        }
            ...
            if (StringUtil.isEmpty(
                        updateAttributeParameterForm.getValue(XML_CODE)
                                                .trim())) {
                updateAttributeParameterForm.setErrorMessage(XML_CODE,
                    "This Field is required.");
                validationStatus = false;
            }
        }
    ...
    {
        String source = updateAttributeParameterForm.getValue(SOURCE);

        updateAttributeParameterForm.setRowVisible(FILEPATH,
            FILEPATH.equals(source));
        updateAttributeParameterForm.setRowVisible(PROPERTY,
            PROPERTY.equals(source));
        updateAttributeParameterForm.setRowVisible(XML_CODE,
            XML_CODE.equals(source));
        updateAttributeParameterForm.setRowVisible(XML_OUTPP,
            XML_CODE.equals(source));
    }
    ...
    @Override public Map<String, String> getValues()
    {
        ...
        Map<String, String> insertFormValues =
            updateAttributeParameterForm.getValues();
            ...
            Collection<ActualParameter> actualParameters)
    {
        ...
        for (String key : new String[] {
            ...
            updateAttributeParameterForm.setValue(key,
    ...
    // -------------------------------------------------------
    interface Binder
        extends UiBinder<Widget, UpdateAttributeParameterPanel> { }
}
